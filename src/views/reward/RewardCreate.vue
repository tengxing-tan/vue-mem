...
const createReward = async () => {
  ...
  await create(toRaw(reward.value))
  router.push('/reward/read')
}

const isValidReward = (): void => {
  isValid.value =
    !!reward.value.name &&
    reward.value.points > 0 &&
    !!reward.value.category &&
    !!reward.value.validFrom &&
    !!reward.value.validUntil
}
</script>
...
<template>
  <section>
    ...
    <form @submit.prevent="createReward" @change="isValidReward">
